...
export default function DashboardCalendar() {
  // State for the Clock (updates every second)
  const [time, setTime] = useState<Date | null>(null);

  // State for the Calendar Widget (user navigation)
  const [date, setDate] = useState<Date | null>(null);

  const [visible, setVisible] = useState<boolean>(false);

  // 1. Setup Locale & Initial State
  useEffect(() => {
    addLocale("id", {
      firstDayOfWeek: 1,
      dayNames: [
  ...
  // Formatting helpers using the 'time' state
  ...
  const secondString = time.toLocaleTimeString("id-ID", {
    second: "2-digit",
  });
  ...
  return (
    <>
      ...
      >
        ...
        <i
          className="pi pi-calendar absolute -bottom-4 -right-4 text-blue-500 opacity-10"
        ...
        <div className="flex flex-column h-full justify-content-center align-items-center relative z-2 gap-3 py-3">
          {/* Bagian Tanggal (Uses 'time' for consistency with clock) */}
          <div className="text-center">
            ...
            <div className="flex align-items-center justify-content-center gap-2 text-gray-800">
              <span className="text-4xl font-bold">{time.getDate()}</span>
          ...
          <div className="text-center">
            ...
            <p
              className="m-0 mt-2 text-xs text-gray-400 font-medium tracking-wide uppercase"
      ...
      >
        <div className="bg-white p-3">
          ...
          <div className="border-top-1 border-gray-100 pt-2 mt-2 flex justify-content-end gap-2">
            <Button
              label="Tutup"
              text
              severity="secondary"
