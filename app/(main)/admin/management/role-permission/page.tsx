"use client";

import InputTextComponent from "@/components/Input";
import { Settings2 } from "lucide-react";
import { Button } from "primereact/button";
import { Calendar } from "primereact/calendar";
import { Card } from "primereact/card";
import DataTableRole from "./components/DataTableRole";
import { useEffect, useMemo, useState } from "react";
import { GetAllRoleData, GetRoleById } from "@/lib/types/role";
import { useFetch } from "@/lib/hooks/useFetch";
import { useToastContext } from "@/components/ToastProvider";
import { useSubmit } from "@/lib/hooks/useSubmit";
import { useDelete } from "@/lib/hooks/useDelete";
import { ConfirmDialog, confirmDialog } from "primereact/confirmdialog";
import { Dialog } from "primereact/dialog";
import RoleDialogForm from "./components/RoleDialogForm";
import { RoleFormData } from "@/lib/schemas/roleFormSchema";
import { useRouter } from "next/navigation";

export default function RolePermissionPage() {
  const [roles, setRoles] = useState<GetAllRoleData[]>([]);
  const [viewRole, setViewRole] = useState<GetRoleById | null>(null);

  const router = useRouter();

  const [currentEditedId, setCurrentEditedId] = useState<number | null>(null);

  const [isDialogVisible, setIsDialogVisible] = useState<boolean>(false);
  const [dialogLabel, setDialogLabel] = useState<
    "Edit Role" | "Tambah Role" | null
  >(null);
  const [dialogMode, setDialogMode] = useState<"add" | "edit" | null>(null);

  const { isLoading, fetchData, fetchDataById } = useFetch();
  const { isSaving, submitData } = useSubmit();
  const deleteData = useDelete();

  const { showToast } = useToastContext();

  const fetchAllRoleData = async () => {
    await fetchData({
      url: "/api/admin/management/role",
      showToast: showToast,
      onSuccess: (responseData) => {
        setRoles(responseData.roles || []);
      },
      onError: () => {
        setRoles([]);
      },
    });
  };

  const fetchRoleById = async (id: number) => {
    await fetchDataById({
      url: `/api/admin/management/role/${id}`,
      onSuccess: (responseData) => {
        setViewRole(responseData.roles || null);
      },
      onError: () => {
        setViewRole(null);
      },
    });
  };

  const cleanRoleDataForm = useMemo(() => {
    if (!viewRole) {
      return null;
    }

    return {
      name: viewRole.name,
      description: viewRole.description,
    };
  }, [viewRole]);

  const handleSubmit = async (formData: RoleFormData) => {
    const url =
      dialogMode === "edit"
        ? `/api/admin/management/role/${currentEditedId}`
        : "/api/admin/management/role";

    const method = dialogMode === "edit" ? "PUT" : "POST";

    const payload = {
      name: formData.name,
      description: formData.description,
    };

    await submitData({
      url: url,
      payload: payload,
      showToast: showToast,
      onSuccess: () => {
        fetchAllRoleData();
        setDialogMode(null);
        setIsDialogVisible(false);
        setCurrentEditedId(null);
      },
      method: method,
    });
  };

  const handleSetting = (role: GetAllRoleData) => {
    router.push(`/admin/management/role-permission/${role.role_code}`);
  };

  const handleEdit = (role: GetAllRoleData) => {
    setDialogMode("edit");
    setIsDialogVisible(true);
    setCurrentEditedId(role.id);
    setDialogLabel("Edit Role");
    fetchRoleById(role.id);
  };

  const handleDelete = (role: GetAllRoleData) => {
    confirmDialog({
      icon: "pi pi-exclamation-triangle text-red-400 mr-2",
      header: "Konfirmasi Hapus",
      message: `Yakin ingin menghapus role ${role.name}?`,
      acceptLabel: "Hapus",
      rejectLabel: "Batal",
      acceptClassName: "p-button-danger",
      accept: async () => {
        await deleteData({
          url: `/api/admin/management/role/${role.id}`,
          onSuccess: () => fetchAllRoleData(),
          showToast: showToast,
        });
      },
    });
  };

  useEffect(() => {
    fetchAllRoleData();
  }, []);
  return (
    <div>
      <div className="mb-6 flex align-items-center gap-3 mt-4 mb-6">
        <div className="bg-blue-100 text-blue-500 p-3 border-round-xl flex align-items-center">
          <Settings2 className="w-2rem h-2rem" />
        </div>
        <div>
          <h1 className="text-lg md:text-2xl font-bold text-gray-800 mb-2">
            Manajemen Pengguna
          </h1>
          <p className="text-sm md:text-md text-gray-500">
            Kelola data diri dan informasi User
          </p>
        </div>
      </div>

      <Card>
        <div className="flex flex-column gap-4">
          <div className="flex gap-2 align-items-center">
            <Settings2 className="h-2" />
            <h2 className="text-base text-800">Manajemen Data Role</h2>
          </div>

          {/* filters */}
          <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-end gap-3">
            {/* calendar */}
            <form className="flex flex-column md:flex-row md:align-items-end gap-3">
              <div className="flex flex-column md:flex-row gap-2">
                {/* start date */}
                <div className="flex flex-column gap-2">
                  <label htmlFor="startDate">Dari</label>
                  <Calendar
                    id="startDate"
                    placeholder="Mulai"
                    showIcon
                    style={{ width: "10rem" }}
                  />
                </div>

                {/* end date */}
                <div className="flex flex-column gap-2">
                  <label htmlFor="endDate">Sampai</label>
                  <Calendar
                    id="startDate"
                    placeholder="Selesai"
                    showIcon
                    style={{ width: "10rem" }}
                  />
                </div>
              </div>

              {/* submit button */}
              <div className="flex flex-column gap-2">
                <span>Terapkan</span>
                <div className="flex align-items-center gap-3">
                  <Button icon="pi pi-check" type="submit" severity="info" />

                  <Button icon="pi pi-times" severity="secondary" />
                </div>
              </div>
            </form>

            {/* search filter and add button */}
            <div className="flex flex-column md:flex-row gap-3">
              {/* search */}
              <InputTextComponent
                icon="pi pi-search"
                placeholder="Cari berdasarkan ID atau nama"
                className="w-full"
              />

              {/* add button */}
              <div>
                <Button
                  icon="pi pi-plus"
                  label="Tambah"
                  severity="info"
                  pt={{
                    icon: { className: "mr-2" },
                  }}
                  onClick={() => {
                    setDialogMode("add");
                    setIsDialogVisible(true);
                    setDialogLabel("Tambah Role");
                    setCurrentEditedId(null);
                  }}
                />
              </div>
            </div>
          </div>

          {/* data table */}
          <DataTableRole
            data={roles}
            isLoading={isLoading}
            onSetting={handleSetting}
            onEdit={handleEdit}
            onDelete={handleDelete}
          />
        </div>

        <ConfirmDialog />

        <Dialog
          header={dialogLabel}
          visible={isDialogVisible}
          onHide={() => {
            setIsDialogVisible(false);
            setDialogLabel(null);
          }}
          modal
          className="w-full md:w-4"
        >
          <div
            className={`${dialogMode === "edit" || dialogMode === "add" ? "block" : "hidden"}`}
          >
            <RoleDialogForm
              roleData={cleanRoleDataForm}
              onSubmit={handleSubmit}
              isSubmitting={isSaving}
            />
          </div>

          {/* <div className={`${dialogMode === "view" ? "block" : "hidden"}`}>
            <UserDialogView
              data={viewUser}
              isLoading={isLoading}
              dialogMode={dialogMode}
            />
          </div> */}
        </Dialog>
      </Card>
    </div>
  );
}
