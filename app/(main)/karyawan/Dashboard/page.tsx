...
const MOCK_EMPLOYEE_DETAILS: EmployeeDetails = {
  ...
  officeAddress:
    "Jl. Margatama Asri IV, Kanigoro, Kec. Kartoharjo, Kota Madiun, Jawa Timur 63118",
...
export default function DashboardRingkasanPage() {
  ...
  const [checkOutStatus, setCheckOutStatus] =
    useState<string>("Belum Check-out");
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      const [scheduleH, scheduleM] = mockJadwalKerja.jamMasuk
        .split(":")
        .map(Number);
  ...
  const handleAttendance = async () => {
    ...
    try {
      ...
      const dist = calculateDistance(
        latitude,
        longitude,
        employeeDetails.officeLat,
        employeeDetails.officeLon
      );
      ...
      if (dist > employeeDetails.maxDistance) {
        ...
        toast.current?.show({
          severity: "error",
          summary: "Jarak Terlalu Jauh",
          detail: "Anda berada di luar area kantor.",
          life: 3000,
        });
        return;
      }
      ...
      setTimeout(() => {
        ...
        if (checkInStatus === "Belum Check-in") {
          ...
          toast.current?.show({
            ...
            detail: `Selamat bekerja! Masuk pukul ${now}`,
            life: 3000,
          });
        } else {
          // Logika Check-Out
          ...
          toast.current?.show({
            ...
            detail: `Terima kasih! Pulang pukul ${now}`,
            life: 3000,
          });
        }
      ...
      toast.current?.show({
        severity: "error",
        summary: "Gagal GPS",
        detail: "Pastikan GPS aktif dan izin lokasi diberikan.",
        life: 3000,
      });
    }
  };
  ...
  const onAttendanceClick = () => {
    ...
    if (!isSessionActive && actionRequired === "Check-in") {
      toast.current?.show({
        severity: "warn",
        summary: "Sesi Belum Dibuka",
        detail: "Silakan tunggu hingga jam absen dibuka.",
        life: 3000,
      });
      return;
    }
