...
export const GET = async (request: NextRequest, { params }: paramsProp) => {
  ...
  try {
    const response = await Axios.get(
      API_ENDPOINTS.GETATTENDANCESESSIONBYID(params.id),
      {
        headers: {
...
export const PUT = async (request: NextRequest, { params }: paramsProp) => {
  ...
  try {
    ...
    const response = await Axios.put(
      API_ENDPOINTS.EDITATTENDANCESESSION(params.id),
      body,
      {
        headers: {
          "Content-Type": "application/json",
    ...
    if (error.response) {
      ...
      return NextResponse.json(
        { message: "Gagal mengedit data sesi absensi" },
        { status: 500 }
      );
    }
  }
...
export const DELETE = async (request: NextRequest, { params }: paramsProp) => {
  ...
  try {
    const response = await Axios.delete(
      API_ENDPOINTS.DELETEATTENDANCESESSION(params.id),
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
    ...
    return NextResponse.json(
      { message: "Gagal menghapus data sesi absensi" },
      { status: 500 }
    );
  }
};
