...
// Consistent helper to check token
...
export const GET = async (request: NextRequest, { params }: paramsProp) => {
  ...
  try {
    const response = await Axios.get(API_ENDPOINTS.GETOFFICEBYID(params.id), {
      headers: {
...
export const PUT = async (request: NextRequest, { params }: paramsProp) => {
  ...
  try {
    ...
    const response = await Axios.put(
      API_ENDPOINTS.EDITOFFICE(params.id),
      body,
      {
        headers: {
          "Content-Type": "application/json",
    ...
    // Return FULL data (crucial for showing validation errors on the form)
    const status = error.response?.status || 500;
    const data = error.response?.data || {
      message: "Gagal mengedit data master kantor",
    };
...
export const DELETE = async (request: NextRequest, { params }: paramsProp) => {
  ...
  try {
    const response = await Axios.delete(API_ENDPOINTS.DELETEOFFICE(params.id), {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      },
    ...
    const status = error.response?.status || 500;
    const data = error.response?.data || {
      message: "Gagal menghapus data master kantor",
    };
