...
export const GET = async ({ params }: paramsProp) => {
	...
	try {
		...
		if (error.response) {
			return NextResponse.json(
				{ message: "Tidak ada data master departemen dengan id tersebut" },
				{ status: 404 }
			);
		}
...
export const DELETE = async (request: NextRequest, { params }: paramsProp) => {
	...
	try {
		...
		if (error.response) {
			return NextResponse.json(
				{ message: "Data master departemen dengan id tersebut gagal dihapus" },
				{ status: 404 }
			);
		}
